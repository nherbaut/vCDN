...
The vHG realize several network functions, but in the present case we will focuse on the routing function.
In this usecase, the vHG can be seen as a transit function able to flag outgoing flows according to L5-L7 rules, like HTTP URL for instance.
Hence vHG can seggregate traffic targetted to vCDN from the one targetted to CDN.
This allows packets to follow distinct routes with distinct network caracteristics.
...
The remaining requests that don't match any URL are sent to a virtual route with no such guarantees, to the CDN AS.
