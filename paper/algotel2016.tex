...
\begin{figure}
	\center
	\includegraphics[width=0.48\textwidth]{fig/overlay.pdf}
...
The canonical representation of Content Delivery Service is depicted Figure~\ref{fig:canonical}, where two distinct flows emerge from the route note $s$. 
...
This limits the possibilities left by the CDNs to choose the righ server, as all the requests sent to a particular domain are resolved by the same DNS request.
...
The vHG realize several network functions, but in the present case we focus on the routing function.
In this usecase, the vHG can be seen as a transit function able to flag outgoing flows according to L5-L7 rules, for instance HTTP URL matching.
vHG can therefore seggregate traffic targetted toward vCDN from the one targetted to CDN.
This enable packets to follow distinct routes with distinct network caracteristics.
For example, for a set of video identified by their URLs, the vHG will route the requests made by the end-users to the given URLs to a vCDN instance following a virtual route with guaranteed bandwidth and maximum delay.
The remaining requests that don't match any URL are sent through a virtual route with no such guarantees to the CDN AS.
...
\begin{figure}
	\center
	\includegraphics[width=1\textwidth]{fig/service-mapping.pdf}
	\caption{ Service Chain Mapping
    \label{fig:service-mapping}}
\end{figure}
The resulting service can now be modelized as a Service Chain depicted in Figure~\ref{fig:vhg}.
...
SLAs varies in the amont of physical resources needed and can be very demanding sometimes. In this context, it can be quite challenging for the ISPs to map a vNF to a single POP or to route the resulting traffic though a single route with enough bandwidth.
To ease embedding into the network, ISP can transform the canonical representation of the service into an other form by splitting vNF and routes as shown Figure~\ref{fig:vhg-lb}. 
