...
In order to overcome this collaboration challenges, we propose a solution capable of matching ISP's connectivity "supply" with the CDN's connectivity "demand".
...
While an SLA is a good way for CDNs to formulate their business needs, it does not specify technically how ISPs should perform the instantiation it on their network.
...
ISPs can benefit their full control of the network to switch to an SDN approach bringing more flexibility.
...
For example, let's assure that a set of video files is cached in a vCDN. Each one is identified by its original URLs in the CP name space. 
...
\begin{figure}
	\center
	\includegraphics[width=.8\textwidth]{fig/service-mapping.pdf}
...
To ease embedding into the network, ISP can transform the canonical representation of the service into an other form by splitting vNF and routes as shown Figure~\ref{fig:vhg-lb} known as path slitting. 
...
Having traffic and resource split, however can ease service mapping on the substrate network.
...
We only considered transformation at the VHG level by increasing the number of vHG being load balanced and fairly spiting the traffic between them. Figure~\ref{fig:results} display the results of increasing the number of vHG while  by picking traffic sources and sinks at random in the graph.
...
We are in the process of implementing a proof of concept of our approach in collaboration with an Operator, into which we will measure the performance in terms of peering traffic reduction, increased quality of service for end-users and scalability. Furthermore, deriving an economic model of this new platform will permit us to analytically evaluate how the equilibrium/balance in revenue sharing is reached and a full win-win approach is achieved.
