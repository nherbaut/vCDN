...
parser.add_argument('--vhgpa-disable', help="Disable grouping vhg by shortest path", dest='vhgpa',
                    action='store_true')
parser.add_argument("--generate_mapping-disable", help="no try to find solution", dest='solve_disable', action='store_true')
...
if args.solve_disable:
    print("Not tried to find a solution (--generate_mapping-disable)")
    print("finish")
    exit(0)
...
if service.mapping is not None:
    ...
    # mapping = generate_mapping(service, su,allow_violations=True)
    # if mapping:
