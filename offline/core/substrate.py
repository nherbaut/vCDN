...
class Substrate:
    ...
    def __fromSpec(cls, args):
        widthcHeight, bw, delay, cpu = args
        width,height=[int(x) for x in widthcHeight.split("x")]
        edges = []
        nodesdict = {}
    ...
    def fromSpec(cls, specs, rs=numpy.random.RandomState()):
        if specs[0] == "grid":
            return cls.__fromSpec(specs[1].split(",") + [10 ** 10, 1, 100])
        elif specs[0] == "file":
            return cls.fromGraph(rs, specs[1][0])
        elif specs[0] == "powerlaw":
            return cls.fromPowerLaw(list(specs[1])  + [10 ** 10, 1, 100])
        elif specs[0] == "erdos_renyi":
            return cls.FromErdosRenyi(list(specs[1])  + [10 ** 10, 1, 100])
        else:
            return cls.__fromSpec([5,5] + [10 ** 10, 1, 100])
