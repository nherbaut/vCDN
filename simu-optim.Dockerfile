...
RUN echo "#!/usr/bin/env bash \n /etc/init.d/mysql start && mysql -u root -proot -h localhost -e 'CREATE database IF NOT EXISTS paper4;' && /opt/simuservice/simu_optim.py --dest_folder=/opt/girafe/results \"\$@\" |tee -a /opt/girafe/results/res.txt "> /opt/simuservice/bootstrap.sh
